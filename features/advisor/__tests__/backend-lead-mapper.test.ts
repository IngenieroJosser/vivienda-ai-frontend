...
describe("backend advisor lead mapper", () => {
  it("maps list fields into the advisor lead contract", () => {
    ...
    expect(lead.backendLead).toEqual(baseLead);
  ...
  it("includes backend nurture leads in the advisor inbox", () => {
    const lead = mapBackendLeadToQualified({
      ...baseLead,
      route: "NURTURE",
      priority: "LOW",
    ...
    expect(opportunities[0]?.lead.evaluation.route).toBe("NURTURE_LONG_TERM");
  });
