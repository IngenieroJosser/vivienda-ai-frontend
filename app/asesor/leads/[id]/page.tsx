    ...
    >
      ...
      <div className="grid gap-5 xl:grid-cols-[1.25fr_.75fr]">
        <div className="space-y-5">
          ...
          <section className="surface-solid p-6 sm:p-8"><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold">Perfil financiero</h2><p className="mt-1 text-xs text-[color:var(--vm-color-ink-muted)]">Información declarada y validada durante el perfilamiento.</p></div><button className="inline-flex items-center gap-2 text-xs font-semibold text-[color:var(--vm-color-brand-blue)]"><Icon name="edit" className="h-4 w-4" />Editar</button></div><div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{[["Ingreso del hogar","$4.800.000"],["Obligaciones","$520.000"],["Ahorros","$18.000.000"],["Cesantías","$13.500.000"],["Cuota disponible","$1.450.000"],["Tipo de contrato","Indefinido"]].map(([label,value]) => <div key={label} className="rounded-[18px] border border-black/[.065] p-4"><div className="text-[10px] uppercase tracking-[.11em] text-[color:var(--vm-color-ink-muted)]">{label}</div><div className="mt-2 text-sm font-semibold">{value}</div></div>)}</div></section>
