    ...
    <main className="mx-auto max-w-[1200px] px-5 py-8 sm:px-8 lg:px-12 lg:py-10">
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <section className="space-y-5">
          ...
          <section className="surface-solid p-6 sm:p-8"><div className="flex items-center gap-3"><Pill tone="yellow">{project.status}</Pill><span className="text-xs text-black/45">Información preliminar</span></div><p className="mt-5 text-sm leading-7 text-black/55">{project.reason}</p><div className="mt-6 grid gap-3 sm:grid-cols-2">{project.features.map((feature) => <div key={feature} className="flex items-center gap-3 rounded-[15px] border border-black/[.06] p-3.5 text-xs font-semibold"><Icon name="check" className="h-4 w-4 text-emerald-700" />{feature}</div>)}</div></section>
        </section>
