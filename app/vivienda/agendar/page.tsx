    ...
    <PublicFlowShell eyebrow="Asesoría personalizada" title="Agenda el siguiente paso." description="Selecciona una fecha, un horario y el canal de atención que prefieras.">
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
        <section className="flow-panel p-6 sm:p-8 lg:p-9">
          <div><h2 className="text-2xl font-bold tracking-[-.04em]">Elige cuándo prefieres conversar</h2><p className="mt-2 text-xs leading-5 text-black/45">Selecciona una opción de preferencia. Un asesor deberá confirmar la disponibilidad antes de considerar la cita agendada.</p></div>
        ...
        <aside className="space-y-4">
          ...
          <section className="rounded-[24px] border border-[color:var(--vm-color-brand-blue)]/15 bg-[linear-gradient(145deg,#eef8ff,#fffdf1)] p-6 shadow-[var(--vm-shadow-medium)]"><div className="flex items-center justify-between"><div className="text-[9px] font-bold uppercase tracking-[.14em] text-[color:var(--vm-color-brand-blue)]">{confirmed ? "Preferencia guardada" : "Resumen de tu solicitud"}</div><Icon name={confirmed ? "check" : "calendar"} className="h-5 w-5 text-[color:var(--vm-color-brand-blue)]" /></div><div className="mt-6 space-y-4">{[["building", "Proyecto", "Versalles"], ["calendar", "Fecha preferida", `${selectedDay?.week} ${day} de julio`], ["clock", "Hora preferida", time], [selectedChannel?.icon ?? "camera", "Modalidad", selectedChannel?.label ?? "Videollamada"]].map(([icon, label, value]) => <div key={label} className="flex items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-[11px] bg-white text-[color:var(--vm-color-brand-blue)] shadow-sm"><Icon name={icon as Parameters<typeof Icon>[0]["name"]} className="h-4 w-4" /></span><div><div className="text-[8px] uppercase tracking-[.1em] text-[color:var(--vm-color-ink-muted)]">{label}</div><div className="mt-1 text-xs font-bold">{value}</div></div></div>)}</div><div className="mt-6 rounded-[15px] bg-white p-4"><div className="flex gap-2.5"><Icon name="info" className="h-4 w-4 shrink-0 text-[color:var(--vm-color-brand-blue)]" /><p className="text-[9px] leading-4 text-[color:var(--vm-color-ink-muted)]">{confirmed ? "Guardamos esta preferencia en la sesión. Un asesor deberá confirmar la disponibilidad por el canal registrado." : "Esto registra una preferencia; todavía no confirma una cita."}</p></div></div>{confirmed ? <button onClick={() => setConfirmed(false)} className="mt-6 inline-flex h-12 w-full items-center justify-center gap-2 rounded-full border border-[color:var(--vm-color-brand-blue)]/20 text-sm font-bold text-[color:var(--vm-color-brand-blue)]">Modificar preferencia</button> : <button onClick={() => setConfirmed(true)} className="mt-6 inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[color:var(--vm-color-brand-blue)] text-sm font-extrabold text-white transition hover:-translate-y-0.5">Guardar preferencia <Icon name="check" className="h-4 w-4" /></button>}</section>
        </aside>
      </div>
    </PublicFlowShell>
  );
