...
export function ProjectCard({ project }: { project: ProjectCatalogItem }) {
  return (
    <article className="project-card surface-card surface-card--interactive group overflow-hidden">
      ...
      <div className="relative h-56 overflow-hidden">
        ...
        <div className="absolute left-4 top-4 flex flex-wrap gap-2">
          <Pill tone="image-overlay">{project.housingTypeLabel}</Pill>
          {project.hasTour ? (
        ...
        <span className="project-card__media-count">
          ...
          {project.mediaCount} recursos para explorar
        </span>
      </div>
      <div className="p-5 sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="text-[11px] font-bold uppercase tracking-[.08em] text-[color:var(--vm-color-brand-blue)]">{project.city} · {project.department}</div>
            <h3 className="mt-1.5 text-2xl font-bold tracking-[-.038em]">{project.name}</h3>
            <p className="mt-1 text-xs text-[color:var(--vm-color-ink-muted)]">{project.development}</p>
          </div>
