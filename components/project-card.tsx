  ...
  return (
    <article className="surface-card surface-card--interactive group overflow-hidden">
      <div className={`relative overflow-hidden ${compact ? "h-44" : "h-56"}`}>
        <Image src={project.image} alt={project.name} fill className="object-cover transition duration-500 group-hover:scale-[1.035]" />
        <div className="absolute inset-0 bg-gradient-to-t from-[#111820]/38 via-transparent to-transparent" />
        ...
        <div className="absolute bottom-4 left-4 rounded-full border border-white/55 bg-white/90 px-3 py-1.5 text-[10px] font-extrabold text-[#0067b1] shadow-sm backdrop-blur">{project.compatibility}% compatible</div>
      </div>
      <div className="p-5 sm:p-6">
        ...
        <Link href={`/vivienda/proyectos/${project.id}`} className="mt-5 inline-flex h-11 w-full items-center justify-center gap-2 rounded-full bg-[#0067b1] text-xs font-bold text-white shadow-[0_10px_24px_rgba(0,103,177,.16)] transition hover:-translate-y-0.5 hover:bg-[#005995]">Ver proyecto <Icon name="arrow" className="h-4 w-4" /></Link>
      </div>
    </article>
  );
}
