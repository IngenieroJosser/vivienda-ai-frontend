...
export function PortalLayout({ role, children }: { role: Role; children: ReactNode }) {
  ...
  const nav = (
    <>
      ...
      <div className="mx-2 mt-7 rounded-[18px] border border-[color:var(--vm-color-line)] bg-gradient-to-br from-[color:var(--vm-color-orientation-sky-soft)] to-white p-3.5">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="text-[9px] font-extrabold uppercase tracking-[.16em] text-[color:var(--vm-color-brand-blue)]">Espacio actual</div>
            <div className="mt-1 truncate text-sm font-bold text-[color:var(--vm-color-ink)]">{config.label}</div>
          </div>
          <span className="grid h-9 w-9 shrink-0 place-items-center rounded-[12px] bg-[color:var(--vm-color-orientation-sky-soft)] text-[color:var(--vm-color-brand-blue)]"><Icon name="briefcase" className="h-4 w-4" /></span>
        </div>
      </div>
        ...
        {config.nav.map((item) => {
          ...
          return (
            <Link
              ...
              className={`group flex items-center gap-3 rounded-[15px] px-3.5 py-3 text-sm transition ${active ? "bg-[color:var(--vm-color-brand-blue)] text-white shadow-[0_10px_26px_rgba(0,103,177,.18)]" : "text-[color:var(--vm-color-ink-muted)] hover:bg-[color:var(--vm-color-orientation-sky-soft)] hover:text-[color:var(--vm-color-brand-blue)]"}`}
            >
              <span className={`grid h-8 w-8 place-items-center rounded-[10px] transition ${active ? "bg-white/12" : "bg-[color:var(--vm-color-orientation-wash)] group-hover:bg-white"}`}><Icon name={item.icon} className="h-[17px] w-[17px]" /></span>
      ...
      <div className="mt-auto px-2 pb-1">
        <Link href="/login" prefetch className="mt-3 flex items-center gap-3 rounded-[15px] px-3.5 py-3 text-xs font-semibold text-[color:var(--vm-color-ink-muted)] transition hover:bg-[color:var(--vm-color-orientation-wash)] hover:text-[color:var(--vm-color-ink)]"><Icon name="logout" className="h-4 w-4" />Cerrar sesión</Link>
      </div>
    </>
  );
          ...
          <header className="sticky top-0 z-30 border-b border-[color:var(--vm-color-line)] bg-white/95">
            <div className="flex h-[64px] items-center justify-between px-5 sm:px-8 lg:px-9">
              <div className="flex items-center gap-3">
                <button onClick={() => setMobileOpen(true)} className="grid h-10 w-10 place-items-center rounded-[13px] border border-[color:var(--vm-color-line)] bg-white shadow-sm lg:hidden"><Icon name="menu" /></button>
              ...
              <div className="flex items-center gap-2.5">
                <div className="flex items-center gap-3 rounded-full border border-[color:var(--vm-color-line)] bg-white py-1.5 pl-1.5 pr-3 shadow-sm">
                  ...
                  <div className="hidden text-left sm:block"><div className="text-[11px] font-bold">Sesión de asesor</div><div className="text-[9px] text-[color:var(--vm-color-ink-muted)]">{config.userRole}</div></div>
                </div>
              </div>
            </div>
          </header>
    ...
    <>
      <section className={`portal-hero portal-hero--${role} mb-4 overflow-hidden rounded-[18px] border border-[color:var(--vm-color-line)] bg-white px-5 py-3.5 shadow-[var(--vm-shadow-low)] sm:px-6 sm:py-4`}>
        <div className="portal-hero__content relative z-10 flex flex-col justify-between gap-4 xl:flex-row xl:items-center">
          <div>
            <div className="mb-1.5 inline-flex items-center gap-2 text-[9px] font-extrabold uppercase tracking-[.15em] text-[color:var(--vm-color-brand-blue)]"><span className="h-1.5 w-1.5 rounded-full bg-[color:var(--vm-color-brand-yellow)]" />{config.label}</div>
